...
import { url } from 'video.js';
  ...
  export default class MyTask extends React.Component {
    ...
    async componentWillMount(){
        const res = await fetch('http://localhost:8080/my_task')
    ...
    beginTask(i){
      ...
      history.push('/user/pictureMarker')
    }
        ...
        return(
            <TableRow
              ...
              >
                ...
                <TableCell>
                    <Button variant="contained" color={is_finished[i]==='false' ? 'primary' : 'error'} onClick={is_finished[i]==='false' ? () => this.beginTask(task_id[i]) : () => this.seeTaskResult(task_id[i])}>
                        {is_finished[i]==='false' ? '开始标注' : '查看结果'}
                    </Button>
                </TableCell>
              </TableRow>
        )
    ...
    render(){
    return (
      <Card title="我领取的任务" bordered={false}>
